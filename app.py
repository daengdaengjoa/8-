...
def 전체글조회():
    ...
    if request.method == "POST":
        ...
        posts = Posting.query.filter(
            getattr(Posting, tag).like(f"%{find}%")).all()
...
def 게시글조회():
    ...
    data1 = {'title': title, 'info': info, 'date': date, 'star': star,
             'nums': nums, 'content': content, 'image_url': image_url}
    print(data1)
